...
class Nightly :
  def __init__(self, group,email,name,b_home):
    ...
    self.TITLE             = "uHAL Nightlies (openstack): %s " % self.pseudo_platform
  ...
  def importCommands(self) :
    ...
    self.COMMANDS += [["INSTALL",
                  ["sed \"s/<platform>/%s/\" uhal.nightly.openstack.repo  | sudo tee /etc/yum.repos.d/uhal.repo > /dev/null" % self.pseudo_platform,
                  "sudo yum clean all",
