...
static char** translate_addresses_buf ( bfd* abfd, bfd_vma* addr, int naddr )
{
  ...
  for ( state=Count; state<=Print; state = tState ( int ( state ) +1 ) )
  {
    if ( state == Print )
    {
    ...
    while ( naddr )
    {
      ...
      else
      {
        ...
        int lStatus ( 0 );
        name = abi::__cxa_demangle ( name , 0 , 0 , &lStatus );
#endif
...
char** backtrace_symbols ( void* const* buffer, int size )
{
  ...
  for ( x=stack_depth, y=0; x>=0; x--, y++ )
  {
    ...
    addr = ( std::size_t ) ( buffer[x] ) - ( std::size_t ) ( match.base );
