...
%% Debug Trace Macro - for simple trace messages %%
%
% It works in a similar fashion to io:format/1 and io:format/2.  Use as follows:
%    DEBUG_TRACE("My message")
%  or something like:
%    DEBUG_TRACE("Debug Value 1 = ~p, Debug Value 2 = ~p.", [DebugValue1, DebugValue2])
% To print a simple trace message.
%
% Either enable the macro globally (see top of this file) or...
% Compile module with c(module_name, {d, debug_trace}) to activate debug trace messages for that module.
%
-ifdef(debug_trace).
...
%% Packet Trace Macro - for putting printing packets for debug purposes %%
%
% Use as follows:
...
% The message will be printed followed by a printout of the packet.
%
% Either enable the macro globally (see top of this file) or...
% Compile module with c(module_name, {d, packet_trace}) to activate packet trace messages for that module.
-ifdef(packet_trace).
