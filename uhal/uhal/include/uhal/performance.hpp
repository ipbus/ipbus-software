...
timeval V1 , V2; \
gPerformanceMeasurement.mDepth++; \
...
class PerformanceMeasurement{
	public:
		...
		struct tPerformanceMeasurement{
			...
			tPerformanceMeasurement( const std::string& aDesc , const std::string& aFunction , const std::string& aFile , const uint32_t& aLine , const timeval& aStartTime , const timeval& aEndTime , const uint32_t& aDepth ):
				mDesc(aDesc),
				mFunction(aFunction),
				mFile(aFile),
				mLine(aLine),
		...
		~PerformanceMeasurement(){
			...
			for( std::deque< tPerformanceMeasurement >::iterator lIt = mPerformanceMeasurement.begin() ; lIt != mPerformanceMeasurement.end() ; ++lIt ){
				mSort[ lIt->mDepth ].push_back( &(*lIt) );
			}
