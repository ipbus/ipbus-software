	...
	ClientFactory& ClientFactory::getInstance()
	{
		try
		{
			if ( mInstance == NULL )
			{
				...
				// ---------------------------------------------------------------------
				// mInstance->add< uhal::ControlHubClient<IPbus_1_3> > ( "chtcp3" , "Hardware access via the Control Hub, using Control Hub Protocol 3 (Single target packets with 48-bit IP+port as identifier) and the default IPbus version which is currently IPbus 1.3" );
				mInstance->add< uhal::ControlHubClient<IPbus_1_3> > ( "chtcp3-1.3" );
